...
// World pack module removed for EN variant — not shipped to OBR's
// official store. (Image cross-reference behavior would conflict with
// OBR's asset hosting / monetization model; we strip it pre-emptively
// to avoid a store-listing rejection.)
...
function supportersHtml(lang: Language): string {
  ...
  const intro = lang === "zh"
    ? "感谢以下支持过作者的小伙伴："
    : "Thanks to everyone who's chipped in to keep this project alive:";
  const enEmpty = lang === "zh"
    ? "暂无英文社区支持者 — 成为第一个吧！"
    : "No English-speaking supporters yet. Be the first!";
  const extra = lang === "zh"
    ? "还有其他没留下名字的支持者们 — 你们让我能继续维持服务器费用和开发这个插件。如果想被列上来，赞助时备注一下昵称就行。"
    : "…and to every supporter who didn't leave a name — you let me keep paying the server bills and shipping new features. Drop a nickname in your tip note if you'd like to be listed.";

  // Each <input> needs a unique id; we rely on the language-of-the-tab
  // version of `supportersHtml` being rendered exactly once per visit
  // to the support tab. Both .body[lang] strings include the same
  // markup so the tab switcher behaves identically regardless of the
  // currently-selected suite language.
  const idEn = "sup-tab-en";
  const idZh = "sup-tab-zh";
  const groupName = "supporter-tabs";

  return `<h3>${heading}</h3>
    <div class="backers-box">
      <p>${intro}</p>
      <div class="supporters-tabs-wrap">
        <input type="radio" name="${groupName}" id="${idEn}" class="sup-radio sup-radio-en" checked>
        <input type="radio" name="${groupName}" id="${idZh}" class="sup-radio sup-radio-zh">
        <div class="supporters-tabs">
          <label for="${idEn}" class="sup-tab sup-tab-en">English (${enCount})</label>
          <label for="${idZh}" class="sup-tab sup-tab-zh">中文 (${zhCount})</label>
        </div>
        <div class="sup-pane sup-pane-en">
          ${enCount === 0
            ? `<p class="sup-empty">${enEmpty}</p>`
            : `<div class="backers">${enList}</div>`}
        </div>
        <div class="sup-pane sup-pane-zh">
          <div class="backers">${zhList}</div>
        </div>
      </div>
      <p class="backers-extra">${extra}</p>
    </div>`;
}
...
const BESTIARY_DESC: BilingualHtml = {
  zh: `<p><b>怪物库</b>，搜索 + 拖动召唤。需配合<b>「库设置」</b>使用 —— 由用户配置内容来源后才有数据。<b>仅 DM 可见</b>。</p>
<ul>
  <li>左侧 tool 栏 <b>${ICONS.dragon}</b> 图标 / 快捷键 <kbd>Shift+A</kbd> 打开</li>
  <li><b>中英文搜索</b> + CR 排序</li>
  ...
  en: `<p><b>Monster library</b> — search + drag-to-spawn. Pairs with the <b>Libraries</b> tab — content is only available after the user configures at least one library. <b>DM only</b>.</p>
<ul>
  <li>Open via the left-rail <b>${ICONS.dragon}</b> tool icon, or shortcut <kbd>Shift+A</kbd></li>
  <li><b>CN/EN search</b> + CR sort</li>
...
const CHARCARD_DESC: BilingualHtml = {
  zh: `<p>导入 / 导出 <b>JSON 角色卡</b> —— 自带 schema，社区可以共享卡片格式。</p>
<ul>
  <li>cluster「<b>角色卡界面</b>」/ <kbd>CapsLock</kbd> 打开全屏面板</li>
  <li>面板里点 <b>导入 JSON</b> 选 .json 文件加载，<b>导出 JSON</b> 一键下载当前卡片</li>
...
<p style="color:#f5c876;font-size:11.5px;margin-top:8px"><b>📱 手机端</b>：全屏面板按钮被隐藏（小屏不可用 + 内存吃紧）。手机玩家仍可通过绑定 token 的小信息框查看。</p>`,
  en: `<p>Import / export <b>JSON character cards</b> — well-defined schema; share files with your group.</p>
<ul>
  <li>cluster's <b>Character Card Panel</b> / <kbd>CapsLock</kbd> opens the fullscreen view</li>
  <li>In-panel: click <b>Import JSON</b> to load a .json file; <b>Export JSON</b> downloads the active card</li>
...
<p><b>搜索 / 角色卡联动</b></p>
<ul>
  <li>搜索 / 怪物图鉴里的所有 <code>{@dice}</code>、<code>{@damage}</code>、<code>{@hit}</code> 等行内标签都可点击直接投</li>
...
<p><b>Search / character card integration</b></p>
<ul>
...
const SEARCH_DESC: BilingualHtml = {
  zh: `<p>右上角搜索栏，对<b>所有已配置库</b>做联想（与<b>「库设置」</b>tab 配合使用）。</p>
<ul>
  <li><b>位置</b>：搜索栏在屏幕<em>哪一侧</em>就向反方向展开；上半屏详情向下，下半屏详情向上</li>
  <li><b>覆盖类别</b>（具体看你的库提供什么）：怪物 / 法术 / 物品 / 职业 / 子职业 / 子职能力 / 种族 / 子种族 / 背景 / 专长 / 灵能 / 状态 / 神祇 / 表格 / 整本书 …</li>
  <li><b>悬停</b>词条 → 右侧浮出详情；<b>点击</b>词条钉住</li>
  <li>玩家能否查怪物由本页下方开关控制</li>
  <li>角色卡 / 怪物面板的特性、法术、武器属性 chip 都能点击 → 自动填入本搜索栏</li>
</ul>
<p style="color:#f5c876;font-size:11.5px;margin-top:8px"><b>📱 手机端</b>：搜索栏不注册（全量索引太占内存）。手机玩家请在桌面/平板上查。</p>`,
  en: `<p>Top-right search bar with autocomplete across <b>every configured library</b> (pairs with the <b>Libraries</b> tab).</p>
<ul>
  ...
  <li><b>Covers</b> (depends on what your libraries provide): monsters / spells / items / classes / subclasses / subclass-features / races / subraces / backgrounds / feats / psionics / conditions / deities / tables / books …</li>
  <li><b>Hover</b> an entry → right pane shows details; <b>click</b> to pin</li>
  <li>Player monster-search gated by the toggle below</li>
  <li>Card / monster-panel feature, spell, and weapon-property chips are all click-to-search</li>
</ul>
<p style="color:#f5c876;font-size:11.5px;margin-top:8px"><b>📱 Mobile</b>: the search bar isn't registered (full-index lookups are too memory-heavy on phones). Use a desktop / tablet.</p>`,
};
...
// =====================================================================
//
// Multi-library support — the user can register one or more
// SRD / homebrew JSON hosts. Custom libraries MUST
...
const AI_PROMPT_TEMPLATE = `You are a D&D 5E data format engineer. Convert the monster / spell / item content I provide below into a JSON file matching the schema described, ready to be loaded via "📁 Local content" without hosting.

你是一个 D&D 5E 数据格式工程师。请把我下面提供的怪物 / 法术 / 物品资料，转换为符合下方说明的 JSON 文件，可直接通过"📁 本地内容"导入（无需托管）。
...
function renderLocalContentBlock(lang: Language): string {
  ...
  const head = lang === "zh"
    ? `
      ...
      <p class="lib-local-desc">把符合下方<b>「内容格式 + AI 助手」</b>规范的 JSON / MD 文件直接导入到本插件，存储在浏览器本地（不上传到任何服务器）。导入的内容会自动合并到搜索框 / 怪物图鉴里。每个客户端独立，DM 端导入的不会自动同步给玩家 —— 想全员共享请使用上面的 URL 库或自己托管。</p>
    `
    : `
      <h3 class="lib-local-h">📁 Local content (no hosting needed)</h3>
      <p class="lib-local-desc">Import JSON / MD files matching the schema in <b>"Content format + AI helper"</b> below. Stored in your browser (never uploaded). Imports merge into search + bestiary automatically. Per-client only — DM imports don't auto-sync to players. For shared content use the URL libraries above.</p>
    `;
  const buttons = isGM ? `
...
function renderLibrariesBody(lang: Language): string {
  ...
  // EN variant: no bundled library, no 5etools-specific copy. Library
  // section starts with a yellow "configure a library" notice + the
  // collapsed format/AI helper block below.
  const head = libs.length === 0 ? (lang === "zh"
    ? `
      <div class="lib-warn" style="background:rgba(245,166,35,0.14);border-color:rgba(245,166,35,0.55);color:#f0b94a">
        ⚠ <b>当前没有任何内容库。</b>怪物图鉴和全局搜索<b>暂不可用</b>，直到至少添加一个库。
        <br>展开下方<b>「内容格式 + AI 助手」</b>了解可接受的 JSON / MD 数据格式，或用本地内容直接导入文件。
      </div>
    `
    : `
      <div class="lib-warn" style="background:rgba(245,166,35,0.14);border-color:rgba(245,166,35,0.55);color:#f0b94a">
        ⚠ <b>No content library configured.</b> Bestiary and Global Search are <b>currently unavailable</b> until at least one library is added.
        <br>Expand <b>"Content format + AI helper"</b> below to see the accepted JSON / MD shape, or use Local Content to import a file directly.
      </div>
    `
  ) : "";
  ...
  const tutorial = lang === "zh" ? `
    <details class="lib-tut">
      <summary>${ICONS.book ?? ""} 内容格式 + AI 助手</summary>
      <div class="lib-tut-body">
        <h4>方式一（推荐）：本地导入 —— 不用托管</h4>
        <p>用 AI 把你的资料转成符合下面规范的 JSON 或 MD 文件，然后用上面的 <b>📁 本地内容 → 导入 JSON / MD</b> 直接选文件加载。整个文件存在你浏览器的 localStorage 里，不上传到任何服务器；导入的内容自动并入搜索框 / 怪物图鉴。每个客户端独立存储 —— 想全员共享请使用方式二。</p>
  ...
  data/
    ...
    items.json, feats.json, ...</code></pre>
        <p>然后在上面的 <b>+ 添加库</b> 里填入 URL（不带末尾斜杠）。本插件会从 search/index.json 自动推断要拉哪些数据文件。</p>
...
{@atk ms} {@hit 5}, reach 5 ft., one target. {@h}{@damage 2d6+3} cold damage.</code></pre>
  ...
  ` : `
    <details class="lib-tut">
      <summary>Content format + AI helper</summary>
      <div class="lib-tut-body">
        <h4>Option A (recommended): Local import — no hosting</h4>
        <p>Use an LLM to convert your raw notes into a JSON or MD file matching the schema below, then click <b>📁 Local content → Import JSON / MD</b>. Files live in your browser's localStorage (never uploaded anywhere). Imports merge into the search bar + bestiary automatically. Per-client only — use Option B if you need to share with players.</p>
...
const TABS: TabDef[] = [
  ...
  },
  {
    ...
    dynamicBody: (lang) => {
      ...
      // EN variant: data-version selector removed (the variant ships
      // without 5etools so there's no 2014/2024 split to gate). Sync
      // character-card list also removed — keeping cross-scene
      // settings sync only.
      return `
        <div class="basics-block">
          ...
          <div class="row">
            <div class="lbl">
              ...
              <div class="desc"><em>${
                lang === "zh"
                  ? "开启后，房间里所有场景共享同一份插件设置（模块开关、库列表等）。开启时会询问是否以当前场景为基准。"
                  : "When ON, every scene in the room shares one set of suite settings (module toggles, libraries…). Enabling prompts whether to use the current scene as the source."
              }</em></div>
            </div>
    ...
    afterRender: (root) => {
      ...
      if (debugBtn) {
        const isOn = (() => {
          try { return localStorage.getItem("full-suite-en/debug-overlay") === "1"; }
          catch { return false; }
        })();
      ...
      // EN variant: data-version selector + cross-scene CC list sync
      // removed from this tab. Only suite-settings sync remains.
      root
  ...
  { id: "focus", zh: `${ICONS.crosshair} 同步视口`, en: `${ICONS.crosshair} Sync Viewport`, moduleId: "focus", body: FOCUS_DESC },
  {
    ...
    dynamicBody: (lang) => {
      ...
      const enabledLibs = libs.filter((l) => l.enabled).length;
      const desc = BESTIARY_DESC[lang];
      const warn = enabledLibs === 0 ? `
        <div class="lib-warn" style="background:rgba(245,166,35,0.14);border-color:rgba(245,166,35,0.55);color:#f0b94a;margin-bottom:12px">
          ⚠ <b>${isZh ? "需要先配置内容库" : "No content library configured"}</b>。
          ${isZh
            ? "请到「<b>库设置</b>」 tab 添加至少一个 URL 库或导入本地内容文件，怪物图鉴才会激活。"
            : "Please go to the <b>Libraries</b> tab and add at least one URL library or import a local content file before this module activates."}
        </div>
      ` : "";
      return warn + desc;
    },
  },
  {
    ...
    // EN variant: only JSON import / export. Removed the legacy
    // xlsx template downloads (those linked to a Chinese-community
    // sheet maintainer's revisions, irrelevant to English users).
    body: CHARCARD_DESC,
  },
  {
    id: "initiative",
    ...
    dynamicBody: (lang) => {
      ...
      const sfxOn = (() => {
        try {
          const v = localStorage.getItem("full-suite-en/sfx-initiative");
          if (v === "0") return false;
          if (v === "1") return true;
          return localStorage.getItem("full-suite-en/sfx-on") !== "0";
        } catch { return true; }
      })();
      return `
    ...
    afterRender: (root) => {
      ...
      root
        ...
        ?.addEventListener("click", (e) => {
          ...
          const next = !wasOn;
          try {
            localStorage.setItem("full-suite-en/sfx-initiative", next ? "1" : "0");
            const diceOn = (() => {
              const v = localStorage.getItem("full-suite-en/sfx-dice");
              if (v === "0") return false;
              if (v === "1") return true;
              return localStorage.getItem("full-suite-en/sfx-on") !== "0";
            })();
            if (next === diceOn) {
              localStorage.setItem("full-suite-en/sfx-on", next ? "1" : "0");
            }
          } catch {}
  ...
  },
  {
    ...
    dynamicBody: (lang) => {
      ...
      // localStorage["full-suite-en/sfx-dice"]; defaults to the legacy
      // "full-suite-en/sfx-on" value if the per-module pref isn't set.
      const sfxOn = (() => {
        try {
          const v = localStorage.getItem("full-suite-en/sfx-dice");
          if (v === "0") return false;
    ...
    afterRender: (root) => {
      root.querySelector<HTMLButtonElement>('.tog[data-key="sfxDice"]')
        ?.addEventListener("click", (e) => {
          ...
          const next = !wasOn;
          try {
            localStorage.setItem("full-suite-en/sfx-dice", next ? "1" : "0");
  ...
  },
  {
    ...
    moduleId: "portals",
    dynamicBody: (lang) => {
      // Per-client localStorage; mirrors com.full-suite-en/portals/blink-enabled.
      const blinkOn = (() => {
        try {
          const v = localStorage.getItem("com.full-suite-en/portals/blink-enabled");
          if (v === "0") return false;
          if (v === "1") return true;
        } catch {}
        return true;
        ...
        ?.addEventListener("click", (e) => {
          ...
          try {
            localStorage.setItem(
              "com.full-suite-en/portals/blink-enabled",
              next ? "1" : "0",
            );
          } catch {}
          btn.classList.toggle("on", next);
  ...
  },
  {
    ...
    dynamicBody: (lang) => {
      ...
      const offset = (() => {
        try {
          const v = localStorage.getItem("com.full-suite-en/bubbles/vertical-offset");
          if (v != null && v !== "") {
            const n = Number(v);
      ...
      // covers everything from compact to chunky.
      const bubbleScale = (() => {
        try {
          const v = localStorage.getItem("com.full-suite-en/bubbles/scale");
          if (v != null && v !== "") {
            const n = Number(v);
...
<p>对于<b>没有绑定角色卡 / 怪物图鉴</b>但仍需要 HP / AC 的 token，右键菜单提供<b>添加血条组件 / 移除血条组件</b>，DM 和玩家都能用。已显示血量气泡的 token <b>选中即弹出</b>编辑面板。DM 端面板有<b>锁定按钮</b>控制玩家可见性。</p>`
...
<p>For tokens with <b>no character-card / bestiary binding</b> that still need HP / AC, the right-click menu offers <b>Add HP Bar / Remove HP Bar</b>. Both DM and players can use it. Tokens that already show HP bubbles just need a select to pop the editor. DM has an extra <b>lock button</b> that gates player visibility.</p>`;
    ...
    afterRender: (root) => {
      ...
      if (offsetInput) {
        const commit = () => {
          ...
          if (raw === "") {
            ...
            try { localStorage.setItem("com.full-suite-en/bubbles/vertical-offset", "-20"); } catch {}
            return;
          }
          const n = Number(raw);
          ...
          try { localStorage.setItem("com.full-suite-en/bubbles/vertical-offset", String(clamped)); } catch {}
        };
      ...
      // Bubble size slider — live-update on input.
      ...
      if (sizeInput) {
        const commit = () => {
          ...
          try { localStorage.setItem("com.full-suite-en/bubbles/scale", String(clamped)); } catch {}
        };
    ...
    dynamicBody: (lang) => {
      ...
      const warn = enabledLibs === 0 ? `
        <div class="lib-warn" style="background:rgba(245,166,35,0.14);border-color:rgba(245,166,35,0.55);color:#f0b94a;margin-bottom:12px">
          ⚠ <b>${isZh ? "需要先配置内容库" : "No content library configured"}</b>。
          ${isZh
            ? "请到「<b>库设置</b>」 tab 添加至少一个 URL 库或导入本地内容文件，全局搜索才会激活。"
            : "Please go to the <b>Libraries</b> tab and add at least one URL library or import a local content file before this module activates."}
        </div>
      ` : "";
      return warn + `
        <h3>${isZh ? "选项" : "Options"}</h3>
        <div class="row">
          <div class="lbl">
            ${isZh ? "允许玩家查询怪物" : "Players Can Search Monsters"}
            <div class="desc">${
              isZh
        ...
        ${!isGM ? `<p class="role-notice">${isZh ? "玩家端只读 · 由 DM 设置" : "Read-only · Set by DM"}</p>` : ""}
        ${SEARCH_DESC[lang]}
      `;
    },
    afterRender: (root) => {
...
<p>Per-client rendering — players only see their tokens' vision, DM sees everything.</p>`,
    ...
    dynamicBody: (lang) => {
      const isZh = lang === "zh";
      const sharedRaw = (() => { try { return localStorage.getItem("com.full-suite-en/vision/shared"); } catch { return null; } })();
    ...
    afterRender: (root) => {
      ...
      btn?.addEventListener("click", () => {
        let cur = true;
        try { cur = localStorage.getItem("com.full-suite-en/vision/shared") !== "0"; } catch {}
        const next = !cur;
        try { localStorage.setItem("com.full-suite-en/vision/shared", next ? "1" : "0"); } catch {}
        btn.classList.toggle("on", next);
        ...
        try {
          window.dispatchEvent(new StorageEvent("storage", {
            key: "com.full-suite-en/vision/shared",
            newValue: next ? "1" : "0",
          }));
        } catch {}
      });
...
function renderContent() {
  ...
  // are present. The old `else if` logic silently masked the static
  // body description for any tab that also had a dynamic widget.
  const parts: string[] = [];
