...
//   a local image into an OBR scene. The user clicks "Add to Library"
...
function loadFromFile(file: File): void {
  ...
  if (file.size > 10 * 1024 * 1024) {
    alert("Image is over 10 MB — too big. Please compress first.");
    return;
  }
...
async function uploadToLibrary(): Promise<void> {
  if (!srcImg) return;
  if (!obrReady) {
    alert("OBR is still initializing, please try again in a moment");
    return;
  }
  ...
  try {
    ...
    setBtnState("idle");
    alert("Failed to bake image: " + (err as Error).message);
    return;
  }
  if (!baked) {
    setBtnState("idle");
  ...
  try {
    ...
    setBtnState("idle");
    alert("Upload to library failed: " + detail);
  }
}
