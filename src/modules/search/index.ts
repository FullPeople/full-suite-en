...
export async function setupSearch(): Promise<void> {
  ...
  // EN variant gate — global search has nothing to query when the
  // user hasn't configured any libraries + has no local content.
  // Skip registration entirely (no popover bar, no top-right icon)
  ...
  try {
    ...
    if (enabledLibs === 0 && localFiles.length === 0) {
      console.info("[search] no libraries configured — skipping setup");
      return;
    }
  } catch {}
