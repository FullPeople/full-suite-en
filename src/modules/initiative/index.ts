...
const DRAG_IN_AUTO_KEY = "full-suite-en/initiative/drag-in-auto";
...
const POPOVER_ID = "com.full-suite-en/initiative-panel";
...
async function registerContextMenus(lang: Lang) {
  ...
  // Stealth toggle removed 2026-05-04 — see ZH version's note. The
  // overlay shader was never finished, so a context menu that flips
  // an invisible flag with no visible effect would be user-hostile.
}
...
export async function setupInitiative(): Promise<void> {
  ...
  // click context menu. Sweep any stale overlay items once on
  // scene ready in case the user is upgrading from a build that
  // left them around.
  unsubs.push(
    OBR.scene.onReadyChange(async (ready) => {
      if (ready) {
