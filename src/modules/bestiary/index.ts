...
const TOOL_ID = "com.full-suite-en/bestiary-tool";
const POPOVER_ID = "com.full-suite-en/bestiary-panel";
const INFO_POPOVER_ID = "com.full-suite-en/bestiary-info";
const TOOL_ACTION_TOGGLE = "com.full-suite-en/bestiary-toggle-shortcut";
...
const CTX_BIND = "com.full-suite-en/bestiary-bind";
const CTX_REBIND = "com.full-suite-en/bestiary-rebind";
const CTX_UNBIND = "com.full-suite-en/bestiary-unbind";
...
const CTX_GROUP_BIND = "com.full-suite-en/bestiary-group-bind";
const CTX_GROUP_UNBIND = "com.full-suite-en/bestiary-group-unbind";

// Picker modal — reuses the bestiary panel HTML with `?pickerForItemId=...`.
const PICKER_MODAL_ID = "com.full-suite-en/bestiary-picker";
...
const MONSTER_DRAG_MODAL_ID = "com.full-suite-en/bestiary-drag-preview";
const MONSTER_DRAG_URL = assetUrl("monster-drag-preview.html");
const BC_MONSTER_DRAG_START = "com.full-suite-en/bestiary-drag-start";
const BC_MONSTER_DROP = "com.full-suite-en/bestiary-drop";
const BC_MONSTER_DRAG_CANCEL = "com.full-suite-en/bestiary-drag-cancel";
...
export async function setupBestiary(): Promise<void> {
  // EN variant gate: if no libraries configured + no local content,
  // bestiary has nothing to show. Skip registration entirely so the
  // tool icon doesn't clutter the OBR sidebar with a panel that
  // would just say "no monsters" forever.
  try {
    const { getState } = await import("../../state");
    const { getLocalFiles } = await import("../../utils/localContent");
    const libs = getState().libraries ?? [];
    const localFiles = getLocalFiles();
    const enabledLibs = libs.filter((l) => l.enabled).length;
    if (enabledLibs === 0 && localFiles.length === 0) {
      console.info("[bestiary] no libraries configured — skipping setup");
      return;
    }
  } catch {}
  try {
    const MIG_KEY = "full-suite-en/bestiary-popup-migration-v2";
  ...
  unsubs.push(
    OBR.broadcast.onMessage("com.full-suite-en/bestiary-shortcut-toggle", () => {
