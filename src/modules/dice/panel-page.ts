...
const BROADCAST_DICE_ROLL = "com.full-suite-en/dice-roll";
...
const LS_HISTORY = "full-suite-en/dice/history";
const LS_LAST_EXPR = "full-suite-en/dice/last-expr";
...
OBR.onReady(async () => {
  ...
  OBR.broadcast.onMessage("com.full-suite-en/dice-panel-fill", (event) => {
    ...
    try { localStorage.removeItem("full-suite-en/dice-pending-prefill"); } catch {}
  });
...
try {
  const pending = localStorage.getItem("full-suite-en/dice-pending-prefill");
  if (pending) {
    setExpression(pending);
    switchTab("roll");
    localStorage.removeItem("full-suite-en/dice-pending-prefill");
    setTimeout(() => exprInput.focus(), 50);
  }
} catch {}
