...
function renderFilters(): void {
  ...
  let html = "";
  if (!editMode) {
    html += `<button class="cat-btn ${activeFilter === null ? "on" : ""}" data-g="">All</button>`;
  }
  ...
  if (editMode) {
    if (addCatPending) {
      html += `<input class="cat-input" id="cat-add-input" type="text" placeholder="New category" maxlength="16"/>`;
    } else {
      html += `<button class="cat-add" id="cat-add-btn" type="button" title="Add category">+</button>`;
    }
  }
...
function renderGrid(): void {
  ...
  let html = "";
  if (!editMode) {
    html += `<div class="bubble eraser" data-id="__clear__">✕  Clear all buffs</div>`;
    ...
    html += `<div class="bubble manage" data-id="__manage__">🛠  Manage buffs</div>`;
  }
  for (const b of list) {
  ...
  if (editMode) {
    html += `<div class="bubble add-pill" id="add-buff-pill">+ New buff</div>`;
  }
...
const EFFECT_LABELS: Array<{ id: BuffEffect; label: string; hint: string }> = [
  { id: "default", label: "Default", hint: "Static bubble (no particle effect)" },
  { id: "float",   label: "Float", hint: "Particles drift up from the token's feet" },
  { id: "drop",    label: "Drop", hint: "Particles fall from the token's top" },
  { id: "flicker", label: "Flicker", hint: "Random twinkling fade in / out" },
  { id: "curve",   label: "Curve", hint: "Curved spread from behind the token (renders below)" },
  { id: "spread",  label: "Spread", hint: "Concentric ripple from token center (renders below)" },
];
...
function openEditPopup(id: string, anchor: HTMLElement): void {
  ...
  const effectsBlock = STATUS_EFFECTS_ENABLED
    ? `
    <div class="pop-fx-label">Experimental · particle effects (GM, desktop only)</div>
    ...
    <div class="pop-row pop-img-row" style="${showImg ? "" : "display:none"}">
      <input class="pop-img-url" type="text"
             ...
             placeholder="Particle image URL (empty = bundled ✨)"/>
      <button class="pop-img-pick" type="button" title="Pick from OBR asset library">📁</button>
    </div>`
    : "";
  popupEl.innerHTML = `
    <div class="pop-row">
    ...
    <div class="pop-row pop-actions">
      ...
      <button class="pop-cancel" type="button">Cancel</button>
      <button class="pop-save" type="button">Save</button>
    </div>
  `;
  ...
  del.addEventListener("click", async () => {
    if (!window.confirm(`Delete "${buff.name}"?`)) return;
...
async function onRenameCategory(oldName: string): Promise<void> {
  const next = window.prompt(`Rename category "${oldName}" (empty = delete)`, oldName);
  if (next === null) return;
  const trimmed = next.trim();
  if (trimmed === "") {
    if (!window.confirm(`Delete category "${oldName}"? Its buffs will move to "${UNCATEGORIZED}".`)) return;
...
const FOOT_APPLY_LINES = [
  `<b>Left-click</b> drag-and-release on a token = apply buff`,
  `<b>Right-click</b> drag across tokens = paint-toggle (apply / remove on each crossing)`,
  `<b>Left-click</b> drag the red ✕ = clear one token`,
  `<b>Right-click</b> drag the red ✕ = paint-clear`,
  `<kbd>]</kbd> close palette`,
];
const FOOT_EDIT_LINES = [
  `<b>Click</b> a category = rename (empty = delete)`,
  `<b>Drag</b> a category = reorder`,
  `<b>Click</b> a buff = edit color / name / effect`,
  `<b>Drag</b> a buff onto a category = recategorise`,
  `<kbd>]</kbd> exit edit mode`,
];
